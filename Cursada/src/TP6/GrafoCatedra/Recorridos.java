...
public class Recorridos<T> {
    ...
    private void dfsReturnLista(int i, Grafo<T> grafo, ListaEnlazadaGenerica<Vertice<T>> lis, boolean[] marca){
        ...
        //variable arista para recorrer la lista de adyacencias del vertice actual
        Arista<T> arista = null;

        //Salva el vertice actual en v
        Vertice<T> v = grafo.listaDeVertices().elemento(i);

        //Agrega el vertice en el ultimo lugar de la lista
        lis.agregarEn(v,lis.tamanio());


        //Salva la lista de adyacencias del vertice actual, la inicializa
        ListaGenerica<Arista<T>> ady = grafo.listaDeAdyacentes(v);
        ady.comenzar();

        //Mientras hayan aristas
        while(!ady.fin()){

            //Salva la arista actual
            arista = ady.proximo();

            //Salva la posicion del vertice destino de esta arista
            j = arista.getVerticeDestino().getPosicion() - 1;

            //Si tal vertice no fue visitado, llama recursivamente pasando la posicion del vertice, el grafo, la lista de vertices, y el arreglo de marcas
            if(!marca[j]){
    ...
    //Retorna la lista de componentes fuertemente conexos como una lista de grafos, implementado con depth first search
    public ListaGenerica<Grafo<T>> ComponentesFuertementeConexos(Grafo<T> grafo){
        ...
        for(int i = 0; i < grafo.listaDeVertices().tamanio(); i++){
            ...
            if (!marca[i]){
                ...
                this.ComponentesFuertementeConexos(i, grafo, marca, CompCon);
    ...
    //ERROR EN LOS CODIGOS DE COMPONENTES FUERTEMENTE CONEXOS, YA QUE ESTAMOS EN UN GRAFO DIRIGIDO
    //DEBERIA VERIFICAR LA DIRECCION HACIA LOS DOS LADOS ANTES DE AGREGAR EL VERTICE AL GRAFO COMPONENTE FUERTEMENTE CONEXO


    //Encontrar los componentes fuertemente conexos de un grafo, implementado con recorrido depth first search
    //Puede retornar los componentes conexos como una lista de grafos
    private void ComponentesFuertementeConexos(int i,Grafo<T> G, boolean[] marca, Grafo<T> CompCon){
        ...
        while(!ady.fin()){
            ...
            //la estrategia que use para comprobar si la conexion entre los vertices es fuerte, parece enredado y podria haber una mejor solucion
            boolean conexionFuerte = false;

            //Salva la lista de adyacentes del vertice al que conecta la arista
            ListaGenerica<Arista<T>> L = G.listaDeAdyacentes(arista.getVerticeDestino());
                
            //Verifica con la lista de ady, si la conexion es fuerte (si la conexion tambien vuelve) entonces el booleano en true
            L.comenzar();   
            while(!L.fin()){
                if(L.proximo().getVerticeDestino() == v) conexionFuerte = true;
            }
            
            //Si tal posicion no está visitada, llamar recursivamente al recorrido dfs con la posicion de destino, el grafo y el arreglo de marcas
            if(!marca[j] && conexionFuerte){
               

                //Conectar el vertice actual v, con el vertice destino SOLO si esa posicion no fue visitada  Y si la conexion de v a w tambien es de w a v   ¿no?
                CompCon.conectar(v, arista.getVerticeDestino());
                

                //Llamado recursivo
                this.ComponentesFuertementeConexos(j, G, marca, CompCon);
            }
        }
