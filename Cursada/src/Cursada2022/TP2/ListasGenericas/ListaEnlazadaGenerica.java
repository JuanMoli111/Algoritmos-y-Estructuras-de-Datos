...
public class ListaEnlazadaGenerica<T> extends ListaGenerica<T> {
	...
	//Recibe un array y lo agrega a la lista, retorna true si se agrego exitosamente, false si hubo una excepcion
	public boolean agregarArray(T[] arr){
		try{
			for(int i = 0; i < arr.length; i++){
				this.agregarFinal(arr[i]);
			}
	...
	//Recibe una lista y la agrega al final de la lista, retorna true si se agrego exitosamente, false si hubo una excepcion
	public boolean agregarLista(ListaEnlazadaGenerica<T> lis){
		try {
			
			lis.comenzar();
			
			while(!lis.fin()){
				this.agregarFinal(lis.proximo());
			}
